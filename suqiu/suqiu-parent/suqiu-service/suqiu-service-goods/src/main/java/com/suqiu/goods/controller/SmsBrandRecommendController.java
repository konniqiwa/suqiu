...
public class SmsBrandRecommendController {
    ...
    public JsonDTO createNew(@RequestBody CreateNewBrandModel model) {
        smsBrandRecommendService.create(model);
        return JsonDTO.createInstance().setStatus(JsonDTO.SUCCESS).setMsg("添加品牌推荐成功");
    }

    @PostMapping(value = "/deleteBrand")
    public JsonDTO deleteNew(@RequestBody DeleteNewBrandModel model) {
        //调用SmsPeopleRecommendService实现根据主键删除
        smsBrandRecommendService.delete(model);
        return JsonDTO.createInstance().setStatus(JsonDTO.SUCCESS).setMsg("删除品牌推荐");
    }

    @PostMapping("/sortBrand/{id}")
    public JsonDTO newSort(@PathVariable Long id, @RequestBody NewSortBrandModel model) {
        smsBrandRecommendService.peopleSort(id, model);
        return JsonDTO.createInstance().setStatus(JsonDTO.SUCCESS).setMsg("设置品牌排序");
    }

    @PostMapping("/isBrandRecommend")
    public JsonDTO isNewRecommend(@RequestBody IsNewRecommendModel model) {
        smsBrandRecommendService.isPeopleRecommend(model);
